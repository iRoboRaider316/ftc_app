...
@Autonomous(name="cf_2_beacon_blue_IL_state", group="LinearOpMode")
//@Disabled

public class cf_2_beacon_blue_IL_state_BACKUP extends LinearOpMode {
    ...
    private void wallTrackBackward() throws InterruptedException {
        ...
        while (bODSensor.getRawLightDetected() < .075 && opModeIsActive()) {
            ...
            telemetry.addData("Range1 value:", fRangeSensor.getDistance(DistanceUnit.CM));
            telemetry.addData("bOD light", (bODSensor.getRawLightDetected()));

            double error = (fRangeSensor.getDistance(DistanceUnit.CM) - 12)/70;
            if (fRangeSensor.getDistance(DistanceUnit.CM) > 100 || fRangeSensor.getDistance(DistanceUnit.CM) < 1)
                error = 0;
    ...
    private void wallTrackForward() throws InterruptedException {
        ...
        while (fODSensor.getRawLightDetected() < .1 && opModeIsActive()) {
            ...
            double error = ((bRangeSensor.getDistance(DistanceUnit.CM) - 12)/70); //60
            if (bRangeSensor.getDistance(DistanceUnit.CM) > 100 || bRangeSensor.getDistance(DistanceUnit.CM) < 1)
                error = 0;
            telemetry.addData("Error", error);
            if (error > 0.1){
                error = 0.1;
    ...
    private void lineUp() throws InterruptedException {
        ...
        while (!done && opModeIsActive()) {
            ...
            error = (fRangeSensor.getDistance(DistanceUnit.CM) - bRangeSensor.getDistance(DistanceUnit.CM))/100;
            telemetry.addData("error", error);
            ...
            telemetry.update();
            if (fRangeSensor.getDistance(DistanceUnit.CM) > 200 || bRangeSensor.getDistance(DistanceUnit.CM) > 200)
                done = false;
            else {
                if (fRangeSensor.getDistance(DistanceUnit.CM) >= bRangeSensor.getDistance(DistanceUnit.CM) -1 && fRangeSensor.getDistance(DistanceUnit.CM) <= bRangeSensor.getDistance(DistanceUnit.CM) +1)
                    done = true;
                else
                    done = false;
            }
