...
public class dev_LinearOpMode_Base extends LinearOpMode {
    ...
    public void launch(double power, DcMotor catapultA, DcMotor catapultB) throws InterruptedException{ //input time as seconds
        catapult1.setTargetPosition(catapult1.getCurrentPosition()+1120);
        catapult2.setTargetPosition(catapult2.getCurrentPosition()+1120);
        catapult1.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        catapult2.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        while (opModeIsActive() && catapultA.isBusy() && catapultB.isBusy()){
            if(catapultA != null) {
                if (catapultA.getCurrentPosition()>(catapultA.getTargetPosition()+30)){
                    catapultA.setTargetPosition(catapultA.getCurrentPosition()+1050);
                }
                else {
                    catapultA.setPower(power);
                }
            }

            if(catapultB != null) {
                if (catapultB.getCurrentPosition()>(catapultB.getTargetPosition()+30)){
                    catapultB.setTargetPosition(catapultB.getCurrentPosition()+1050);
                }
                else {
                    catapultB.setPower(power);
                }
            }
        ...
        catapultA.setPower(0);
        catapultB.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
        catapultB.setMode(DcMotor.RunMode.RUN_WITHOUT_ENCODER);
    }
    ...
    public void driveToLine() throws InterruptedException {// this code has no parameters.
        ...
        while(!ODCondition) {                       // unlimited loop which has a break statement later
            if(rODFoundLine()) {
                if(lODFoundLine()) {
                    lDrive1.setPower(0);
                    lDrive2.setPower(0);
    ...
    {
        robotName = ""; // set name of Robot(i.e. velociraptor or crossfire)

        switch(robotName) { // runs ONCE when
            case "velociraptor":
                // code for Velociraptor goes here
                break;
            case "crossfire":
                // code for Crossfire goes here
                break;
            default:
                telemetry.addData("Can't find Robot name!", 0); // runs when robot name isn't set
                sleep(5000);
                break;
        }
    }
