...
@Autonomous(name="cf_auto_red_SIUC", group="LinearOpMode")

public class cf_auto_red_SIUC extends LinearOpMode {
    ...
    private void driveToWall() throws InterruptedException {
        ...
        while((range2Cache[0] & 0xFF) > 14) {
            range1Cache = RANGE1Reader.read(RANGE1_REG_START, RANGE1_READ_LENGTH);
            range2Cache = RANGE2Reader.read(RANGE2_REG_START, RANGE2_READ_LENGTH);
            telemetry.addData("Range1 value:", (range1Cache[0] & 0xFF) );
            telemetry.addData("Range2 value:", (range2Cache[0] & 0xFF) );
            telemetry.update();
        }
        telemetry.addData("Range1 end value:", (range1Cache[0] & 0xFF) );
    ...
    public void runOpMode() throws InterruptedException {
        ...
        int drift;
        drift = (gyroSensor.getHeading() - 340) * -1;
        if (drift < 0){
            drift = drift * -1;
        }
        telemetry.addData("Heading", drift);
        updateTelemetry(telemetry);
        ...
        direction = -1;
        heading = 340;
        drive(distance, maxSpeed, direction, heading + drift);
        driveToWall();
        gyroTurn(drift, 0.4, -1);
        //lineUpWithWall();
        // Remember that the current heading is parallel to the wall
        parallel = gyroSensor.getHeading();
        // Drive backward to the white line
        driveBackwardToLine(parallel + drift);
        // Drive forward 1 in to accommidate for overshooting
        distance = 1;
        maxSpeed = .4;
        direction = 1;
        drive(distance, maxSpeed, direction, parallel + drift);
        // Detect the beacon color and push the correct button
        direction = -1;
        recognizeColorRed(direction);
        // Drive forward 1 in to get past the line
        distance = 1;
        maxSpeed = .4;
        direction = 1;
        drive(distance, maxSpeed, direction, parallel + drift);
        ...
        maxSpeed = .4;
        direction = 1;
        gyroTurn(targetHeading + drift, maxSpeed, direction);
        // Drive forward 1 inch
        distance = 2;
        maxSpeed = .4;
        direction = 1;
