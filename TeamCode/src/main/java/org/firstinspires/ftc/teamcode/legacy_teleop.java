...
@TeleOp(name="legacy_teleop", group="TeleOp")
//@Disabled
public class legacy_teleop extends OpMode{

    public DcMotor lfDriveM, lbDriveM, rfDriveM, rbDriveM, glyphLiftM, relicRotateM, relicExtendM;
    public Servo lGlyphS, rGlyphS, jewelExtendS, jewelKnockS, relicArmS, relicMoldS;
    public CRServo glyphSlideS;

    //public VoltageSensor voltage;         // for later uses
    ...
    String relicArmToggle = "inOn";
    String relicMoldToggle = "inOn";
    ...
    public void init() {
        ...
        //Glypher left-to-right servo
        glyphSlideS = hardwareMap.crservo.get("glyphSlideS");
        glyphSlideS.setPower(0);

        // Relic motors
        relicRotateM = hardwareMap.dcMotor.get("relicRotateM");
        relicRotateM.setPower(0);
        relicExtendM = hardwareMap.dcMotor.get("relicExtendM");
        ...
        // Relic Grabber Servos
        relicArmS = hardwareMap.servo.get("relicArmS");
        relicMoldS = hardwareMap.servo.get("relicMoldS");
...
//        jewelKnockS = hardwareMap.servo.get("jewelKnockS");
    ...
    public void loop() {
        ...
        relicRotateM.setPower(gamepad2.left_stick_x);
        relicExtendM.setPower(gamepad2.left_stick_y);

        // Relic Arm Toggle Case
        if (gamepad2.left_bumper && relicArmToggle == "inOn") {                // Once B is held...
            relicArmToggle = "inOff";
        } else if (relicArmToggle == "inOff" && !gamepad2.left_bumper) {       // Once B is released...
            relicArmS.setPosition(1);                                          // Open Relic Arm
            relicArmToggle = "outOn";
        } else if (gamepad2.left_bumper && relicArmToggle == "outOn") {        // Once B is held...
            relicArmToggle = "outOff";
        } else if (relicArmToggle == "outOff" && !gamepad2.left_bumper) {      // Once B is released...
            relicArmS.setPosition(0);                                          // Retract Relic Arm
            relicArmToggle = "inOn";
        }

        // Relic Mold Thingy Toggle Case (idk what it's called)
        if (gamepad2.right_bumper && relicMoldToggle == "inOn") {              // Once Y is held...
            relicMoldToggle = "inOff";
        } else if (relicMoldToggle == "inOff" && !gamepad2.right_bumper) {     // Once Y is released...
            relicMoldS.setPosition(1);                                         // Grab the Relic!
            relicMoldToggle = "outOn";
        } else if (gamepad2.right_bumper && relicMoldToggle == "outOn") {      // Once Y is held...
            relicMoldToggle = "outOff";
        } else if (relicMoldToggle == "outOff" && !gamepad2.right_bumper) {    // Once Y is released...
            relicMoldS.setPosition(0);                                         // Release the Relic!(55 points! YEAH! Wait, what? We didn't grab it??? sighhh...)
            relicMoldToggle = "inOn";
        }
